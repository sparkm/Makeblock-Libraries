...
MePort::MePort(void)
{
  _pin[OFFSET_0] = mePort[0]._pin_0;
  _pin[OFFSET_1] = mePort[0]._pin_1;
  _pin[OFFSET_2] = mePort[0]._pin_2;
  _port = 0;
}
...
MePort::MePort(uint8_t port)
{
  ...
  _pin[OFFSET_1] = mePort[port]._pin_1;
  _pin[OFFSET_2] = mePort[port]._pin_2;
  _port = port;
}
 ...
 *   None
 */
bool MePort::dRead(uint8_t mode, uint8_t offset)
{
  bool val;
  pinMode(_pin[offset], mode);
  val = digitalRead(_pin[offset]);
  return(val);
}
 ...
 *   None
 */
void MePort::dWrite(bool value, uint8_t offset)
{
  pinMode(_pin[offset], OUTPUT);
  digitalWrite(_pin[offset], value);
}
 ...
 *   None
 */
int16_t MePort::aRead(uint8_t offset)
{
  int16_t val;
  pinMode(_pin[offset], INPUT);
  val = analogRead(_pin[offset]);
  return(val);
}
 ...
 *   None
 */
void MePort::aWrite(int16_t value, uint8_t offset)
{
  analogWrite(_pin[offset], value);
}
 ...
 *   None
 */
uint8_t MePort::getPin(uint8_t offset)
{
  return (_pin[offset]);
}
 ...
 *   None
 */
uint8_t MePort::getPin(uint8_t port, uint8_t offset)
{
  uint8_t val;

  switch (offset) {
    case OFFSET_1:
    val = mePort[port]._pin_1;
    break;

    case OFFSET_2:
    val = mePort[port]._pin_2;
    break;

    default:
    val = mePort[port]._pin_0;
  }
